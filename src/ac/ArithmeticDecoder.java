...
public class ArithmeticDecoder<T> {
	...
	public ArithmeticDecoder(int rangeBitWidth) {
	...
	public T decode(SourceModel<T> model, BitSource bit_source) throws IOException, InsufficientBitsLeftException {
		...
		for (int i = 0; i < model.size(); i++) {
			sym_low = _low + ((long) (range_width * model.cdfLow(i)));
			sym_high = _low + ((long) (range_width * model.cdfHigh(i))) - 1L;
		...
		while (highBit(_low) == highBit(_input_buffer) && highBit(_high) == highBit(_input_buffer)) {
		...
		while (_high < _three_quarter_mark && _low > _one_quarter_mark) {
			...
			_high = ((_high & (_range_mask >> 2)) << 1) | // Shift all but top
															// two over by 1
			...
			_input_buffer = ((_input_buffer & (_range_mask >> 2)) << 1) | // Shift
																			// all
																			// but
																			// top
																			// two
					...
					(top_input_bit << (_range_bit_width - 1)); // Restore the 1
																// on top
		}
